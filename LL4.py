...
"""
A method to remove the last node from a linked list
"""
class Node:
...
class LinkedList:
    ...
    def pop(self):
        ...
        temp = self.head 
        pre = self.head
        while temp.next is not None: #if temp points to a node
            pre = temp # set pre to current node temp
            temp = temp.next #set temp to the next node
            
        self.tail = pre #after looping pre becomes the tail of the list
        self.tail.next = None #update the tail of the list to point to None to detach node
        self.length -= 1 #decrease length of the list
        
        if self.length == 0: # if the list becomes empty after removing the node
